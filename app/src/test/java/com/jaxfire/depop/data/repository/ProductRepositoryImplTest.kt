...
class ProductRepositoryImplTest : KoinTest {
    ...
    @Test
    fun `GIVEN RemoteDataSource throws a NoConnectivityException WHEN the popular products are requested THEN a ResultWrapper of type NetworkConnectivityError is returned`() {
        declareMock<RemoteDataSource> {
            runBlocking {
                given(getPopularProducts()).willThrow(NoConnectivityException::class.java)
